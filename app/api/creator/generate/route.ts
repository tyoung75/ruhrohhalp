...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 2b. Deduplicate: multiple models often converge on the same topics.
    //     Cluster similar posts and keep only the best from each cluster.
    const { kept, dedupRemoved } = await deduplicatePosts(candidates);

    // 3. Safety audit + brand voice audit (run in parallel) — only on deduplicated set
    const dedupPosts = kept.map((c) => c.post);
    const [auditResults, brandVoiceResults] = await Promise.all([
      auditPosts(dedupPosts),
      auditBrandVoice(dedupPosts, context),
    ]);
    ...
    for (let i = 0; i < dedupPosts.length; i++) {
      const post = dedupPosts[i];
      const modelSource = kept[i]?.source ?? "internal";
...
/**
 * Deduplicate posts that cover the same topic/angle across models.
 * Sends all candidates to Claude to cluster by theme and pick the best per cluster.
 */
async function deduplicatePosts(
  candidates: GeneratedCandidate[]
): Promise<{ kept: GeneratedCandidate[]; dedupRemoved: number }> {
  if (candidates.length <= 5) {
    // Single model output — no cross-model duplication possible
    return { kept: candidates, dedupRemoved: 0 };
  }

  try {
    const postList = candidates
      .map((c, i) => {
        const bodyText = Array.isArray(c.post.body) ? c.post.body.join(" | ") : c.post.body;
        return `[${i}] (${c.source}, confidence: ${c.post.confidence ?? "?"}) ${bodyText}`;
      })
      .join("\n\n");

    const dedupPrompt = `You have ${candidates.length} social media posts generated by multiple AI models for the same creator. Many cover the SAME topic or angle — these are redundant.

Your job: identify clusters of posts about the same core topic/theme, and from each cluster pick the SINGLE best post (most specific, funniest, most authentic). Posts that are unique topics — keep them.
...
Return ONLY a JSON object:
{
  "keep": [0, 3, 7, ...],
  "clusters": [
    { "topic": "App Store review queue", "posts": [0, 5, 9], "best": 0, "reason": "most specific and funny" },
    ...
  ]
}

Rules:
- Two posts are redundant if a reader would think "didn't I just read this?" — same joke, same observation, same data point
- A running post and a different running post about different aspects are NOT redundant
- Threads (multi-part) and single posts on the same topic ARE redundant — pick the better format
- When in doubt, keep both — only remove clear duplicates
- "keep" must include ALL indices you want to keep (including unique posts not in any cluster)`;

    const raw = await callClaude(dedupPrompt, "", 1024, { temperature: 0 });
    ...
    try {
      ...
      if (!objMatch) throw new Error("No JSON in dedup response");
      parsed = JSON.parse(objMatch[0]);
    }

    if (!Array.isArray(parsed.keep) || parsed.keep.length === 0) {
      throw new Error("Dedup returned empty keep list");
    }
    ...
    console.log(`[creator-generate] Dedup: ${candidates.length} → ${kept.length} (removed ${removed} redundant)`);

    return { kept, dedupRemoved: removed };
  } catch (error) {
    console.error("[creator-generate] Dedup failed, keeping all:", error);
    return { kept: candidates, dedupRemoved: 0 };
  }
}
