...
const ADVISOR_MEMORY_TABLE = "financial_advisor_memory";

function advisorMemorySelect(supabase: Awaited<ReturnType<typeof createClient>>, userId: string) {
  return supabase
    .from(ADVISOR_MEMORY_TABLE)
...
export async function GET() {
  ...
  const [accountsRes, holdingsRes, incomeRes, debtsRes, contribsRes, rsusRes, configRes, statementsRes, memoryRes] = await Promise.all([
    ...
    advisorMemorySelect(supabase, user.id),
  ]);
...
Respect monthly decision cadence (do not recommend daily trading). Use historicalMemory feedback to adapt recommendations.`;
  ...
  const { error: insertError } = await supabase.from(ADVISOR_MEMORY_TABLE).insert({
    user_id: user.id,
...
export async function POST(request: Request) {
  ...
  const supabase = await createClient();
  const { error } = await supabase.from(ADVISOR_MEMORY_TABLE).insert({
    user_id: user.id,
